...
result = np.load("out_recording2.npy")
print(result.shape)

print(result.shape, result.dtype)

res = result.reshape((3*64, 48828))
print(res.shape)
out = res
fs = 48828
...
def main():
    ...
    max_value_ok = 1.25*10**5 #np.max(np.max(data[0:4000,],axis=0)) # maximum value of data, to use for axis scaling in plots
    ...
    if plot_selected:
        ...
        plot_mics = list(range(8, 17))#[8, 18, 60, 64]                     # what microphones to plot
