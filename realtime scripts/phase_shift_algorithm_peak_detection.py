...
threshold_upper = 0.4            # threshold value for detecting peak values (set between 0 and 1)
...
freq_threshold_idx = (np.abs(f - freq_threshold)).argmin()
...
def peak_detection(power):
    ...
    for f_ind in range(freq_threshold_idx, int(n_samples/2)+1):
        if (np.max(power[f_ind,:,:]) > threshold_upper*np.max(power) > threshold_lower):
...
def main(signal):
    ...
    #FFT_shifted_modes = frequency_phase_shift_modes(signal)
    #power_list = list(map(calc_power, FFT_shifted_modes, active_mics_mode_list))
    #heatmap = peak_detection_modes(power_list)
    #heatmap = (heatmap/(np.max(heatmap)+0.0001))**7
