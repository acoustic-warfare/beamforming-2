...
if config.data_type == 'emulated':
    def generate_filename():
        if config.sources == 1:
            filename ='emul_'+ 'samples='+str(config.N_SAMPLES) + '_'+ str(config.f_start1)+'Hz_'+'theta='+str(config.theta_deg1)+'_phi='+str(config.phi_deg1)+ \
                '_A'+str(config.ACTIVE_ARRAYS)
        elif config.sources == 2:
            filename ='emul_'+'samples='+str(config.N_SAMPLES) + '_'+str(config.f_start1)+str(config.f_start2)+'Hz_'+'theta='+str(config.theta_deg1)+str(config.theta_deg2)+ \
...
def frequency_phase_shift(signal, phase_shift):
    print(signal.shape)
    print(calc_phase_shift_cartesian.active_mics)
...
FFT_power /= config.N_MICROPHONES
