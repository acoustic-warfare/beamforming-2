...
# signal[:,:] = 0 + np.random.uniform(0,np.max(signal)/2, (config.ELEMENTS, config.N_SAMPLES)).T
...
alpha = config.VIEW_ANGLE    # total scanning angle (bildvinkel) in theta-direction [degrees], from config
...
fs = config.SAMPLE_RATE         # sample rate
N = config.N_SAMPLES            # number of samples
